 ...
 * 					========
 * 					~VO(값만 가지고 있는 클래스), ~DTO
 * 					~Manager , ~Service , ~DAO(데이터 베이스에서 관련된 데이터를 가져오는 클래스)
 * 	============
 ...
 *				백두산 (2019)
				...
				=> int a
				   double d
				   ============== 컴퓨터가 메모리 크기 인식
				   MovieVO vo	new가 없었을때 malloc(sizeOf(MovieVO))
				   ========== 메모리 크기를 알 수 없다
				   		메모리 크기 확인 => 메모리를 생성
				   		====================== new
...
class MovieVO
{
	// instance 변수 => 메모리가 저장되는 시점 ==> new(새로운 메모리를 만들때 => 따로 메모리 생성)
	String title;
	double score;
	String genre;
	String regdate;
